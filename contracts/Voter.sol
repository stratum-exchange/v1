...
contract Voter is IVoter, Constants {
  ...
  mapping(address => bool) public is3poolGauge;
  ...
  function createGauge(
    ...
    address _external_bribe = IBribeFactory(bribefactory).createExternalBribe(
      allowedRewards
    );
    ...
    IWrappedExternalBribeFactory(_wxBribeFactory).createBribe(_external_bribe);
  ...
  function createGauge3pool(
    ...
    address[] memory internalRewards = new address[](_token.length);
    for (uint256 i = 0; i < _token.length; i++) {
      allowedRewards[i] = _token[i];
      internalRewards[i] = _token[i];
    }
    allowedRewards[_token.length] = base;
    ...
    internal_bribes[_gauge] = _internal_bribe;
    external_bribes[_gauge] = _external_bribe;
    gauges[_3pool] = _gauge;
    poolForGauge[_gauge] = _3pool;
    isGauge[_gauge] = true;
    isAlive[_gauge] = true;
    _updateFor(_gauge);
    pools.push(_3pool);
  ...
  function distributeFeesFor3Pool(
    address[] memory _gauges,
    address[] memory _swapAddress
  ) external {
    for (uint i = 0; i < _gauges.length; i++) {
      if (IGauge(_gauges[i]).isForPair()) {
        IGauge(_gauges[i]).claimFeesFor3Pool(_swapAddress[i]);
      }
    }
  }
