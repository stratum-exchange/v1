...
contract VotingEscrow is IERC721, IERC721Metadata, IVotes, Constants {
  enum DepositType {
    DEPOSIT_FOR_TYPE,
    CREATE_LOCK_TYPE,
  ...
  /// @notice Restrictions that are active until an expiry timestamp, when veSTRAT is lent/vested using lend()
  struct LendingRestrictions {
    address lender; // the lender of this veNFT (can optionally pull back)
    uint expiresAt; // no restriction (see below) after this timestamp
  ...
  function approve(address _approved, uint _tokenId) public {
    require(_restrictions_permit_approve(_tokenId), "approve restricted");
  ...
  uint internal constant WEEK = SECONDS_PER_EPOCH;
  ...
  function _create_lock(
    ...
    require(
      ...
      "Voting lock can be 1 years max"
    );
  ...
  function increase_unlock_time(
    ...
    require(
      _restrictions_permit_increase_unlock_time(_tokenId),
      "increase_unlock_time restricted"
    );
    assert(_isApprovedOrOwner(msg.sender, _tokenId));
  ...
  function withdraw(uint _tokenId) external nonreentrant {
    assert(_isApprovedOrOwner(msg.sender, _tokenId));
    require(_restrictions_permit_withdraw(_tokenId), "withdraw restricted");
    require(attachments[_tokenId] == 0 && !voted[_tokenId], "attached");
  ...
  function merge(uint _from, uint _to) external {
    require(_restrictions_permit_merge(_from), "merge restricted for _from");
    require(_restrictions_permit_merge(_to), "merge restricted for _to");
  ...
  function delegateBySig(
    ...
    // require(
    //   SignatureChecker.isValidSignatureNow(delegatee, digest, signature),
    //   "S"
    // );
    require(
      signatory != address(0),
  ...
  /*///////////////////////////////////////////////////////////////
                 LENDING RESTRICTIONS FUNCTIONALITY
  //////////////////////////////////////////////////////////////*/

  // Use Cases:
  // 1.) Temporary Lending of veSTRAT to projects
  // 2.) Retroactive Vesting: Team members receive veSTRAT for previous work. Can't sell or withdraw but
  //     already vote/earn etc. but not yet sell, transfer, merge, withdraw or similar. Can't get pulled back by
  //     stratum anymore. Full possession/features after expiry of lending restrictions without further ado.
  // 3.) Optimistic Vesting: Similar as (2). Team members receive lent/restricted veSTRAT for future activity
  //     and good behaviour in advance. Member can already vote/earn etc. but not transfer, merge, etc.
  //     In case of misbehaviour multisig could also pull back that veSTRAT, so members will stay aligned & motivated.
  // 4.) Stepwise linear vesting of 'unlocked' 50% STRAT allocation of team members could optionally be split
  //     e.g. into 4x veSTRAT locked and lent for 3, 6, 9 and 12 months. So they are already unlocked, when going into
  ...
  ///         to pull back the veSTRAT.
  ///         Typical lending use case parameters: _noWithdraw=true, _noApprove=true,
  ///           _noIncreaseUnlockTime=upToYou, _noMerge=true (warning: borrower could merge it to
  ///           an unrestricted veSTRAT, effectively removing lending restrictions!!)
  function lend(
    uint _tokenId,
    address _to,
    uint _expiresAt,
    ...
    if (!_noPullback) {
      // enforce those restrictions that could otherwise lead to loss
  ...
  ///         is the issuer of the restrictions
  ...
  function _restrictions_permit_withdraw(
    uint _tokenId
  ) internal view returns (bool) {
    return !restrictions[_tokenId].noWithdraw || !isBorrowed(_tokenId);
  }

  function _restrictions_permit_merge(
    uint _tokenId
  ) internal view returns (bool) {
    return !restrictions[_tokenId].noMerge || !isBorrowed(_tokenId);
  }

  function _restrictions_permit_increase_unlock_time(
    uint _tokenId
  ) internal view returns (bool) {
    return
      !restrictions[_tokenId].noIncreaseUnlockTime || !isBorrowed(_tokenId);
  }

  function _restrictions_permit_approve(
    uint _tokenId
  ) internal view returns (bool) {
    return !restrictions[_tokenId].noApprove || !isBorrowed(_tokenId);
  }

  function _restrictions_permit_transfer(
    uint _tokenId,
    address _to
  ) internal view returns (bool) {
    return
